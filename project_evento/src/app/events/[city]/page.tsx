...
const EventsPage = async ({ params }: EventsPageProps) => {
  ...
  return (
    <main className="flex flex-col items-center py-24 px-[20px] min-h-[110vh]">
      ...
      <Suspense fallback={<Loading />}> {// Suspense is used to handle the loading state of the EventsList component. While the data is being fetched, we can show a fallback UI (like a loading spinner) to indicate that the content is loading. Once the data is fetched, the EventsList component will be rendered with the fetched events.
      }
